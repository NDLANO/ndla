...
class ConverterService {
  ...
  def toApiAlternative(a: Alternative, isStaff: Boolean): AlternativeDTO = AlternativeDTO(
    id = a.id,
    text = a.text,
    isCorrect =
      if (isStaff) Some(a.isCorrect)
      else None,
  )

  def toApiQuestion(q: Question, isStaff: Boolean): QuestionDTO = QuestionDTO(
    id = q.id,
  ...
  def toDomainQuiz(dto: NewQuizDTO, user: String, now: NDLADate, language: String): Quiz = Quiz(
    id = None,
    revision = None,
  ...
  def mergeQuestion(existing: Question, dto: UpdateQuestionDTO, now: NDLADate): Question = existing.copy(
