...
class HtmlTagRulesTest extends UnitSuite {
  test("embed tag should be an allowed tag and contain data attributes") {
    ...
    val dataAttrs       = TagAttribute.values.map(_.toString).filter(x => x.startsWith("data-"))
  ...
  test("span tag should be an allowed tag and contain one lang attribute") {
    ...
    val dataAttrs = TagAttribute
      .values
      .map(_.toString)
      .filter(x => x.startsWith("lang") && x != TagAttribute.DataType.toString)
