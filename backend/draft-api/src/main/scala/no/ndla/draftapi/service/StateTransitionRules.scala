...
class StateTransitionRules(using
  ...
  private val DirectPublishRoles: Set[Permission] = PublishRoles + Permission.DRAFT_API_ADMIN
    ...
    val StateTransitions: mutable.LinkedHashSet[StateTransition] = mutable.LinkedHashSet(
      ...
      (PUBLISHED             -> UNPUBLISHED)           `keepStates` Set.empty `require` PublishRoles `withSideEffect` unpublishArticle `withSideEffect` resetResponsible,
      (PUBLISHED             -> ARCHIVED)              `keepStates` Set.empty `require` DirectPublishRoles `withSideEffect` unpublishArticle `withSideEffect` resetResponsible,
      REPUBLISH              -> REPUBLISH,
