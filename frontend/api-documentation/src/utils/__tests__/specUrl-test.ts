...
it("allows specs served from the api domain", () => {
  expect(
    isAllowedSpecUrl("https://api.test.ndla.no/article-api/api-docs", prod),
  ).toBe(true);
  expect(isAllowedSpecUrl("https://api.test.ndla.no", prod)).toBe(true);
});

it("rejects specs from any other origin", () => {
  expect(isAllowedSpecUrl("https://evil.example.com/api-docs", prod)).toBe(
    false,
  );
  expect(isAllowedSpecUrl("http://api.test.ndla.no/api-docs", prod)).toBe(
    false,
  );
  expect(
    isAllowedSpecUrl(
      "https://api.test.ndla.no.evil.example.com/api-docs",
      prod,
    ),
  ).toBe(false);
  expect(isAllowedSpecUrl("https://api.test.ndla.no:8080/api-docs", prod)).toBe(
    false,
  );
  expect(
    isAllowedSpecUrl(
      "https://evil.example.com/?x=https://api.test.ndla.no",
      prod,
    ),
  ).toBe(false);
});
...
it("allows localhost only when localhost is allowed", () => {
  ...
  expect(
    isAllowedSpecUrl("http://evil.localhost.example.com/api-docs", local),
  ).toBe(false);
});
