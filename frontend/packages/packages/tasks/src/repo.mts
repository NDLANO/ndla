...
export const run = (dir: string, ...command: string[]): number =>
  spawnSync("mise", ["exec", "--", ...command], { cwd: dir, stdio: "inherit" })
    .status ?? 1;

export const mill = (...command: string[]): number =>
  run(backend, "./mill", ...command);
export const nx = (...command: string[]): number =>
  run(frontend, "pnpm", "exec", "nx", ...command);
...
const services = (): string[] =>
  find(backend, "package.mill", "DockerComponent");
...
export const javaModules = (): string[] =>
  find(backend, "package.mill", "JavaBaseModule");
...
const nxProjects = (script: string): string[] =>
  readdirSync(frontend)
    ...
    .flatMap((pkg) =>
      pkg.name !== undefined && pkg.scripts?.[script] !== undefined
        ? [pkg.name]
        : [],
    )
    .sort();
...
export const frontendProjects = (scope: string): string[] =>
  nxProjects(scope === "dev" ? "dev" : "test");
export const backendProjects = (scope: string): string[] =>
  scope === "dev" ? services() : testable();
