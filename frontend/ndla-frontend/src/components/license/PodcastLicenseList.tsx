...
const PodcastLicenseInfo = ({ podcast }: PodcastLicenseInfoProps) => {
  ...
  const items: ItemType[] = getGroupedContributorDescriptionList(podcast.copyright, t);
    ...
    <MediaListItem>
      ...
      >
        <MediaListContent>
          ...
          {!isCopyrighted(podcast.copyright.license.license) && (
            <MediaListItemActions>
              <SafeLinkButton to={downloadUrl(podcast.src)} download variant="secondary" size="small">
                <DownloadLine />
                {t("license.download")}
              </SafeLinkButton>
              <CopyTextButton
