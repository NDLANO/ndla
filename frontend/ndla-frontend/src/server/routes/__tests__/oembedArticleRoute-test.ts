...
test("oembedArticleRoute success", async () => {
  nock("http://ndla-api")
    ...
    .times(2)
    .reply(200, { id: "urn:resource:1", contentUri: "urn:article:123", name: "Resource title" });

  const response = await oembedArticleRoute({ query: { url: validArticleUrl1 } } as any as Request);

  expect(response).toMatchSnapshot();

  const response2 = await oembedArticleRoute({ query: { url: validArticleUrl2 } } as any as Request);
...
test("oembedArticleRoute learningpath fail", async () => {
  nock("http://ndla-api")
    .get("/taxonomy/v1/nodes/urn:resource:1:9876?language=nb")
    .reply(200, { id: "urn:resource:1:9876", contentUri: "urn:learningpath:123", name: "Learningpath title" });

  const response = await oembedArticleRoute({ query: { url: validLearningpathUrl } } as any as Request);
...
test("oembedArticleRoute invalid url", async () => {
  const response = await oembedArticleRoute({ query: { url: unvalidArticleUrl } } as any as Request);
...
describe("parseAndMatchUrl", () => {
  ...
  it("supports topic articles with old URL scheme", () => {
    ...
    expect(result).toEqual({ subjectId: "urn:subject:3", topicId: "urn:topic:1:55163" });
  });
  it("supports language prefix with old URL scheme", () => {
    ...
    expect(result).toEqual({ subjectId: "urn:subject:3", topicId: "urn:topic:1:55163", lang: "nn" });
  });
  it("defaults to nb language prefix when encountering unknown languages with old URL scheme", () => {
    ...
    expect(result).toEqual({ subjectId: "urn:subject:3", topicId: "urn:topic:1:55163", lang: "nb" });
  });
  it("supports nesting of topics with old URL scheme", () => {
    ...
    expect(result).toEqual({ subjectId: "urn:subject:3", topicId: "urn:topic:1:168554" });
  });
  ...
  it("supports article-iframe with taxonomy id", () => {
    ...
    expect(result).toEqual({ articleId: "1200", nodeId: "urn:resource:1:89279" });
  });
  ...
  it("supports plain embed paths", () => {
    ...
    expect(h5pId).toEqual({ h5pId: "4809" });
  });
  it("supports plain embed paths with language prefix", () => {
    ...
    expect(h5pId).toEqual({ h5pId: "4809", lang: "nn" });
  });
