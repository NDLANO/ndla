...
test("transformUrlIfNeeded returns static nrk url for new nrk embed format", async () => {
  const url = await transformUrlIfNeeded(
    "https://tv.nrk.no/program/OUHA24000304",
  );
  expect(url).toMatchSnapshot();
});
...
test("transformUrlIfNeeded returns codepen embed url", async () => {
  ...
  const url2 = await transformUrlIfNeeded(
    "https://codepen.io/team/user/pen/rzqYxX",
  );
...
test("transformUrlIfNeeded replaces www with embed for ted.com", async () => {
  const url1 = await transformUrlIfNeeded(
    "https://www.ted.com/talks/some_fancy_talk",
  );
...
test("transformUrlIfNeeded returns original for flourish", async () => {
  ...
  const url2 = await transformUrlIfNeeded(
    "https://flo.uri.sh/visualisation/8515737/embed",
  );
...
test("transformUrlIfNeeded adds /embed for flourish", async () => {
  const url1 = await transformUrlIfNeeded(
    "https://public.flourish.studio/visualisation/8515737",
  );
  expect(url1).toMatch(
    "https://public.flourish.studio/visualisation/8515737/embed",
  );
  const url2 = await transformUrlIfNeeded(
    "https://flo.uri.sh/visualisation/8515737",
  );
  expect(url2).toMatch("https://flo.uri.sh/visualisation/8515737/embed");
});
...
test("transformUrlIfNeeded transforms url for sketchfab", async () => {
  ...
  expect(url1).toMatch(
    "https://sketchfab.com/models/7573f836dd3a46bdbce8b90b5a40f104/embed",
  );
});
...
test("transformUrlIfNeeded strips ndlafilm for filmiundervisning.no", async () => {
  const url1 = await transformUrlIfNeeded(
    "https://ndla.filmiundervisning.no/film/ndlafilm.aspx?filmId=400199",
  );
