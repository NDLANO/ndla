...
// used. See the first import in `index.ts`. The operator-injected auto-instrumentation cannot patch this
// ESM app (it relies on a CommonJS `--require` hook), so we run the SDK in-process instead.
