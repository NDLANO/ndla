...
export const createDraft = async (draft: NewArticleDTO): Promise<ArticleDTO> =>
  client
    .POST("/draft-api/v1/drafts", { body: draft })
    .then((r) => resolveJsonOATS(r));

export const searchDrafts = async (
  query: ArticleSearchParamsDTO,
): Promise<ArticleSearchResultDTO> =>
  client
    .POST("/draft-api/v1/drafts/search", { body: query })
    .then((r) => resolveJsonOATS(r));
...
export const fetchArticleRevisionHistory = async (
  id: number,
  language?: string,
...
export const deleteLanguageVersion = async (
  id: number,
  language: string,
): Promise<ArticleDTO> =>
  client
...
export const fetchNewArticleId = async (
  id: number,
): Promise<{ id: number }> => {
  return client
...
export const updateUserData = async (
  userData: UpdatedUserDataDTO,
): Promise<UserDataDTO> =>
  client
    .PATCH("/draft-api/v1/user-data", { body: userData })
    .then((r) => resolveJsonOATS(r));
...
export const fetchDraftResponsibles = async (): Promise<string[]> =>
  client
    .GET("/draft-api/v1/user-data/responsibles")
    .then((r) => resolveJsonOATS(r));
...
export const migrateCodes = async (): Promise<void> => {
  return client
    .POST("/draft-api/v1/drafts/migrate-greps")
    .then((r) => resolveOATS(r));
};
