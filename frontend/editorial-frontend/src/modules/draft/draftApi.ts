...
export const fetchBySlug = async (slug: string, language?: string): Promise<ArticleDTO> => {
  return client
    .GET("/draft-api/v1/drafts/slug/{slug}", { params: { path: { slug }, query: { language, fallback: true } } })
    .then((r) => resolveJsonOATS(r));
};
...
export const cloneDraft = async (
  ...
  client
    .POST("/draft-api/v1/drafts/clone/{article_id}", {
      params: {
        path: { article_id: id },
        query: { language, "copied-title-postfix": addCopyPostfixToArticleTitle, fallback: true },
      },
    })
    .then((r) => resolveJsonOATS(r));
...
export const deleteLanguageVersion = async (id: number, language: string): Promise<ArticleDTO> =>
  client
    .DELETE("/draft-api/v1/drafts/{article_id}/language/{language}", { params: { path: { article_id: id, language } } })
    .then((r) => resolveJsonOATS(r));

export const fetchNewArticleId = async (id: number): Promise<{ id: number }> => {
  return client
    .GET("/draft-api/v1/drafts/external_id/{deprecated_node_id}", { params: { path: { deprecated_node_id: id } } })
    .then((r) => resolveJsonOATS(r));
};

export const validateDraft = async (id: number, draft: UpdatedArticleDTO): Promise<{ id: number }> =>
  client
    .PUT("/draft-api/v1/drafts/{article_id}/validate", { body: draft, params: { path: { article_id: id } } })
    .then((r) => resolveJsonOATS(r));

export const updateStatusDraft = async (id: number, status: DraftStatus): Promise<ArticleDTO> =>
  client
    .PUT("/draft-api/v1/drafts/{article_id}/status/{STATUS}", { params: { path: { article_id: id, STATUS: status } } })
    .then((r) => resolveJsonOATS(r));

export const fetchSearchTags = async (input: string, language: string): Promise<TagsSearchResultDTO> =>
  client
    .GET("/draft-api/v1/drafts/tag-search", { params: { query: { language, query: input, fallback: true } } })
    .then((r) => resolveJsonOATS(r));
...
export const fetchStatusStateMachine = async (id?: number): Promise<Record<DraftStatus, DraftStatus[]>> =>
  client
    .GET("/draft-api/v1/drafts/status-state-machine", { params: { query: { articleId: id } } })
    .then((r) => resolveJsonOATS(r))
    .then((data) => data as Record<DraftStatus, DraftStatus[]>);

export const copyRevisionDates = (nodeId: string): Promise<void> =>
  client
    .POST("/draft-api/v1/drafts/copyRevisionDates/{node_id}", { params: { path: { node_id: nodeId } } })
